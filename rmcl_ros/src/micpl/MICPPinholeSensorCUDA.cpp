...
MICPPinholeSensorCUDA::MICPPinholeSensorCUDA(
...
void MICPPinholeSensorCUDA::connectToTopic(const std::string& topic_name)
{
  ...
  tf_filter_->registerCallback(&MICPPinholeSensorCUDA::updateMsg, this);



  RCLCPP_INFO_STREAM(nh_->get_logger(), "[" << name << "] [MICPPinholeSensorCUDA] Waiting for message from topic '" << topic_name << "'...");
}

void MICPPinholeSensorCUDA::getDataFromParameters()
{
  static_dataset = true;
...
void MICPPinholeSensorCUDA::updateMsg(
  const rmcl_msgs::msg::DepthStamped::SharedPtr msg)
{
  ...
  { // print conversion & sync delay
    RCLCPP_DEBUG_STREAM(nh_->get_logger(), "[" << name << "::topicCB] MICPPinholeSensorCUDA Timings:");
...
void MICPPinholeSensorCUDA::unpackMessage(
  const rmcl_msgs::msg::DepthStamped::SharedPtr msg)
{
  /////
  // sensor model
  // std::cout << "INFO: " << msg->depth.info.range_min << ", " << msg->depth.info.range_max << std::endl;
