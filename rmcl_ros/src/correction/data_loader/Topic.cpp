...
void TopicSourceO1Dn::topicCB(const rmcl_msgs::msg::O1DnStamped::SharedPtr msg)
{
  ...
  for(size_t i = 0; i<n_outer_; i++)
  {
    ...
    {
      rm::Transform T_snew_sold = ~Tsb * T_bnew_bold * Tsb; // from snew to bnew to bold to sold -> snew to sold
      ...
      // recover actual bnew to bold
      T_bnew_bold = Tsb * T_snew_sold * ~Tsb; // from bnew to to snew to sold to bold -> bnew to bold
    }
