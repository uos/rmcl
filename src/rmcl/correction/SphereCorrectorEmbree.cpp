...
{
    CorrectionPreResults<RAM> pre_res = pre_correct(Tbms);

    Memory<Matrix3x3, RAM> Us(Tbms.size());
    Memory<Matrix3x3, RAM> Vs(Tbms.size());

    m_svd->calcUV(pre_res.Cs, Us, Vs);
    auto Rs = rm::multNxN(Us, rm::transpose(Vs));
    auto ts = rm::subNxN(pre_res.ds, rm::multNxN(Rs, pre_res.ms));
    ...
    res.Ncorr = std::move(pre_res.Ncorr);
...
CorrectionPreResults<rmagine::RAM> SphereCorrectorEmbree::pre_correct(
    const rmagine::MemoryView<rmagine::Transform, rmagine::RAM>& Tbms)
{
    ...
    for(size_t pid=0; pid < Tbms.size(); pid++)
    {
        ...
        if(Ncorr > 0)
        {
            ...
            res.ms[pid] = {to_mean.x(), to_mean.y(), to_mean.z()};
            res.ds[pid] = {from_mean.x(), from_mean.y(), from_mean.z()};
            res.Cs[pid] = *reinterpret_cast<const Matrix3x3*>(&C);
        } else {
            res.ms[pid] = {0.0, 0.0, 0.0};
            res.ds[pid] = {0.0, 0.0, 0.0};
            res.Cs[pid].setZeros();
        }
    }
